...
struct Task* PingTask = 0L;
BlankerPrefs* Prefs;
BYTE ProgName[108];
LONG RetVal;

int appClassInit(void)
{
    struct Process* Proc;

    Proc = (struct Process*)ServerTask;

    if (!Proc->pr_CLI) strcpy(ProgName, WBenchMsg->sm_ArgList->wa_Name);
    else               strcpy(ProgName, "PROGDIR:Garshneblanker");

    if (OpenLibraries() == 1L) return 6L;

    if (CheckCX() == QUIT) return 6L;

    ServerPort = CreatePort("GarshneServer", 0);
    TimerPort = CreatePort(0L, 0);

    if (!ServerPort || !TimerPort) return 5L;

    TimeOutIO = (struct timerequest*)
        CreateExtIO(TimerPort, sizeof(struct timerequest));
    if (!TimeOutIO) return 4L;
...
void appClassDestruct(void)
{
    ...
    case 4:
        ...
        if (ServerPort) DeletePort(ServerPort);
    case 5:
        ShutdownCX();
    case 6:
        CloseLibraries();
...
int main(void)
{
    ...
    if (Prefs->bp_PopUp) RetVal = OpenInterface();

    do {
        Sigs = Wait(SIG_CX | ISigs() | SIG_SERVPORT | SIGBREAKF_CTRL_C | 
					SIGBREAKF_CTRL_D);
        
        if (Sigs & SIG_SERVPORT) RetVal = HandleServerMsg();

        if (Sigs & SIG_CX) RetVal = HandleCxMess();

        if (Sigs & SIGBREAKF_CTRL_C) RetVal = 0L;

        if (Sigs & SIGBREAKF_CTRL_D) RetVal = HandleMouseCheck();
                        
		if (Sigs & ISigs()) RetVal = HandleInterface();
        ...
        case RESTART:
            ...
            Prefs = LoadDefaultPrefs();
            BlankerEntries = LoadBlankerEntries(Prefs->bp_Dir);
            if ((RetVal = SetupCX()) == OK) RetVal = OpenInterface();
            break;
        }
    } while(RetVal);
